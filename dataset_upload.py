...
class erpf_dataset_manager():
    ...
    def __init__(self, engine_url, api_key, output_files_location):
        engine_url = engine_url[:-1] if engine_url.endswith('/') else engine_url
        self.dataset_engine = CkanDatasetEngine(endpoint='%s/api/action' % engine_url, apikey=api_key)
    ...
    def make_tarfiles(self, source_dir, watershed):
        ...
        for basin_file in basin_files:
            try:
                basin_name = basin_name_search.search(basin_file).group(1).lower()
